<%@ Page Language="VB" AutoEventWireup="false" CodeFile="forms.aspx.vb" Inherits="addbank" %>
    ...
    function validation1() {
        ...
        if ($('#abr').val() == '') { showMessage('abr cannot be empty', 'abr'); $('#abr').focus(); return false; }
            ...
            if (ans != true) {
                ...
                $("#frmbankname").attr("action", "?tbl=tblbank&task=<% response.write(keyp) %>&lrd=empcontener&key=id&keyval=<% response.write(idx) %>&rd=empcontener.aspx&" + str);
                $("#frmbankname").submit();
                return true;
            }
        }
...
<div></div>
    ...
    <% If Session("username") = "" Then
       ...
       If Session("bank_name") = "" Then
       %>

<script type="text/javascript">
    //document.location.href="admin_home.php"
    window.location = "bankname.aspx";
</script>
...
<table><tr><td><input type='hidden' id='bank_name' name='bank_name' value='<%response.write(session("bank_name")) %>' />abr<sup style='color:red;'>*</sup></td>
<td>:</td><td><textarea id='abr' name='abr' cols="30" rows="10"></textarea><label class='lblsmall'>eg. Driving, C++,Java, House Maintainance, etc...</label></td>
...
<input type='hidden' id='date_reg' name='date_reg' value="<% dim lucur(3) as string
 ...
 response.write(sdate) %>" />publish<sup style='color:red;'>*</sup></td><td>:</td><td><br /><label class='lblsmall'></label></td>
 ...
 <div id="listx" style=" width:100%; padding:0px 7px 0px 0px;">
    <%  Dim db As New dbclass
      ...
      If dt.HasRows = True Then
        ...
        Dim sqlx As String = "select id,bank_name,abr from tblbanks where bank_name='" & Session("bank_name") & "' order by id desc"
