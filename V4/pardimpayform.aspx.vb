...
Partial Class pardimpayform
    ...
    Function mkformview(ByVal projid As String, ByVal fdate As Date, ByVal edate As Date)
        ...
        If fdate <> "1/1/1900" Then
            ...
            rs = ds.dtmake("selectpar", "select * from pardimpay inner join emprec on emprec.id=pardimpay.emptid INNER JOIN emp_static_info AS empsf ON emprec.emp_id = empsf.emp_id " & _
                          "where paid_state='y' and paid_date between '" & fdate & "' and '" & edate & "' order by pardimpay.ref,paid_date,pardimpay.id,empsf.first_name", Session("con"))
        ...
        Else
            ...
            rs = ds.dtmake("mmkj", "SELECT pardimpay.* FROM pardimpay INNER JOIN emprec ON pardimpay.emptid = emprec.id INNER JOIN emp_static_info ON emprec.emp_id = emp_static_info.emp_id ORDER BY pardimpay.ref,pardimpay.paid_date, emp_static_info.first_name", Session("con"))
        ...
        If rs.HasRows = True Then
            ...
            While rs.Read
                ...
                'Response.Write("<br>" & emptid & "=====" & fdate & "=====" & edate & "=======" & projid & "=" & rt(0))
                If rt.Length > 0 Then
