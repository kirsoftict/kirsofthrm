<%@ Page Language="VB" AutoEventWireup="false" CodeFile="education.aspx.vb" Inherits="education" %>
...
var requf=["emp_id","qualification","diciplin","school","country","year_g","stat","x"];   
var fieldlist=["emp_id","qualification","diciplin","school","country","year_g","stat","x"];
  var prv;
  prv="";
      var id;
  var focused="";
...
if ($('#diciplin').val() == '') {showMessage('diciplin cannot be empty','diciplin');$('#diciplin').focus();return false;}
if ($('#school').val() == '') {showMessage('school cannot be empty','school');$('#school').focus();return false;}
if ($('#country').val() == '') {showMessage('country cannot be empty','country');$('#country').focus();return false;}
if ($('#year_g').val() == '') {showMessage('year_g cannot be empty','year_g');$('#year_g').focus();return false;}
if ($('#stat').val() == '') {showMessage('stat cannot be empty','stat');$('#stat').focus();return false;}
if ($('#score').val() == '') {showMessage('Score cannot be empty','score');$('#score').focus();return false;}
 ...
 { //alert("called validation");
   ...
   var str=$("#frmemp_education").formSerialize();
 //alert(str);
 ...
 // alert("open.aspx?tbl=emp_education&task=save&lrd=empcontener&key=emp_id&rd=empcontenener.aspx&" + str);
    $("#frmemp_education").attr("action","?tbl=emp_education&task=<% response.write(keyp) %>&lrd=empcontener&key=id&keyval=<% response.write(idx) %>&rd=empcontener.aspx&" + str);
    
   // $("#messagebox").load("open.aspx?tbl=emp_education&rd=&" + str);
    $('#frmemp_education').submit();
...
If Request.Form.HasKeys = True Then
    ...
    ' Dim sql As String
    ' sql = db.makest("emp_education", Request.Form, session("con"), "")
    'Response.Write(sql)
...
<span class="close" id="clickclose_s" style="cursor:pointer;"></span>
    ...
    <span id="messageboxx"></span>
           <form method='post' id='frmemp_education' name='frmemp_education' action=""> 
<table><tr><td><input type='hidden' id='emp_id' name='emp_id' value="<% response.write(session("emp_id")) %>" />
Qualification<sup style='color:red;'>*</sup></td><td>:</td>
<td>
<select id='qualification' name='qualification'>
    <option value="">--Select--
    </option>
<%  Dim fm As New formMaker
    Response.Write(fm.getoption("tblqualification", "Qualification", "qualification", session("con")))%>
</select>
</td>
</tr><tr>
<td>Field of Study<sup style='color:red;'>*</sup></td><td>:</td>
<td><select id='diciplin' name='diciplin' >
 <option value="">--Select--
    </option>
<%  
    Response.Write(fm.getoption("tbldiscipline", "discipline", "discipline", session("con")))%>
</select>

</td></tr><tr>
<td>College/University<sup style='color:red;'>*</sup></td><td>:</td>
<td colspan="4"><input type='text' id='school' name='school' style="width:350px;" /></td>
</tr><tr><td>Country<sup style='color:red;'>*</sup></td><td>:</td>
<td><select id='country' name='country'>
<option value="">--Select--
    </option>
<%  
    Response.Write(fm.getoption("tblcountry", "country", "country", session("con")))%>
</select>

</td>
<td>Year of Graduate<sup style='color:red;'>*</sup></td><td>:</td>
<td><select id='year_g' name="year_g">
<option value="">--Select year--</option>
    <% for i as integer = today.year to today.year-70 step -1
            Response.Write("<option value='" & i & "'>" & i & "</option>")
    next  %>
</select></td></tr><tr>
<td>Score<sup style='color:red;'>*</sup></td><td>:</td>
<td colspan="4"><input type='text' id='score' name='score' style="width:350px;" />
<br /><label class="lblsmall">eg. 74% or 2.75 out of 4</label></td>
</tr><tr><td>stat<sup style='color:red;'>*</sup></td><td>:</td>
<td><input type='text' id='stat' name='stat' value='y' /></td>
</tr><tr><td colspan='4'><input type='hidden' id='date_reg' name='date_reg' value="<% dim lucur(3) as string
...
<input type='reset' onclick="javascript:$('#btnSave').attr('title','Save');$('#btnSave').attr('value','Save');" /></td></tr></table></form>
 ...
 <div id="listx" style=" width:100%; padding:0px 7px 0px 0px;">
    <%  Dim db As New dbclass
        ...
        If keyp = "update" Then
            dt = db.dtmake("new" & Today.ToLocalTime, "select * from emp_education where id=" & Request.QueryString("id"), session("con"))
        ...
        Dim sqlx As String = "select id,qualification,diciplin,school,country,year_g,score from emp_education where emp_id='" & Session("emp_id") & "' order by id desc"
        row = mk.edit_del_list("emp_education", sqlx, "Qualification,Field Of Study, College/University,Country,Year of Graduate,Score", session("con"), loc)
            Response.Write(row)
      ...
      function del(val,ans,hd)
       {
        ...
        $('#frmx').attr("action","<% response.write(loc) %>?task=delete&id="+val+"&tbl=emp_education");
        $('#frmx').submit();
            }
            else
            {
