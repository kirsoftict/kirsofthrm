<%@ Page Language="VB" AutoEventWireup="false" CodeFile="empreferance.aspx.vb" Inherits="empReference" %>
...
var id;
var focused="";
var requf=["emp_id","refname","mob","x"];
var fieldlist=["emp_id","refname","position","email","mob","tel","who_reg","date_reg","x"];
function validation1(){if ($('#emp_id').val() == '') {showMessage('emp_id cannot be empty','emp_id');$('#emp_id').focus();return false;}
if ($('#refname').val() == '') {showMessage('refname cannot be empty','refname');$('#refname').focus();return false;}
...
if(ans!=true){ 
 $("#" + ans).focus();
   ...
   var str=$("#frmempReference").formSerialize();
   $("#frmempReference").attr("action","?tbl=empreferance&task=<% response.write(keyp) %>&lrd=empcontener&key=id&keyval=<% response.write(idx) %>&rd=empcontener.aspx&" + str);
    $("#frmempReference").submit();
  return true;}
  }
} </script>
...
<span class="close" id="clickclose_s" style="cursor:pointer;"></span>
  ...
  <form method='post' id='frmempReference' name='frmempReference' action=""> 
<table><tr><td><input type='hidden' id='emp_id' name='emp_id' value='<%response.write(session("emp_id")) %>' />
Reference Full Name<sup style='color:red;'>*</sup></td><td>:</td>
<td><input type='text' id='refname' name='refname' value='' /><br />
<label class='lblsmall'></label></td>
</tr><tr><td>Position<sup style='color:red;'></sup></td><td>:</td><td>
<input type='text' id='position' name='position' value='' />
<br /><label class='lblsmall'></label></td>
<td>Email<sup style='color:red;'></sup></td><td>:</td><td><input type='text' id='email' name='email' value='' />
<br /><label class='lblsmall'>username@domain.com</label></td>
</tr><tr><td>Mob<sup style='color:red;'>*</sup></td><td>:</td><td><input type='text' id='mob' name='mob' value='' />
<br /><label class='lblsmall'></label></td>
<td>Tel<sup style='color:red;'></sup></td><td>:</td><td><input type='text' id='tel' name='tel' value='' />
<br /><label class='lblsmall'></label></td>
...
<input type='hidden' id='date_reg' name='date_reg' value="<% dim lucur(3) as string
 ...
 response.write(sdate) %>" /><input type='button' name='btnSave' id='btnSave' value='Save' /><input type='reset' onclick="javascript:$('#btnSave').attr('title','Save');$('#btnSave').attr('value','Save');" /></td></tr></table></form>
    </div>
 ...
 <div id="listx" style=" width:100%; padding:0px 7px 0px 0px;">
    <%  Dim db As New dbclass
        ...
        If keyp = "update" Then
            ...
            dt = db.dtmake("new" & Today.ToLocalTime, "select * from empreferance where id=" & Request.QueryString("id"), Session("con"))
      If dt.HasRows = True Then
        ...
        Dim sqlx As String = "select id,refname,position,email,mob from empreferance where emp_id='" & Session("emp_id") & "' order by id desc"
        row = mk.edit_del_list("empreferance", sqlx, "Ref. Name,Position, Email, Mob No.", Session("con"), loc)
            Response.Write(row)
      ...
      function del(val,ans,hd)
       {
        ...
        $('#frmx').attr("action","<% response.write(loc) %>?task=delete&id="+val+"&tbl=empreferance");
        $('#frmx').submit();
            }
            else
            {
