...
"<img src='images/gif/exportxls.gif' height='28px' style='float:left;' alt='excel' /> Export to Excel</div>"
    ...
    Function first_page_title(ByVal pgid As String)
        ...
        rtn &= "<tbody><tr>"
        rtn &= "  <td align='center' colspan='2'><img src='http:\\" & Session("url") & "/css/Employment Tax_files/fdre_logo.jpg' width='66'></td>"
...
rtn &=         "            <div class='table_title'>ክፍል 2 - ማስታወቂያ ዝርዝር መረጃ</div>"
    ...
    Function collectiononly()
            ...
            If rs.HasRows Then
                ...
                While rs.Read
                    ...
                    Else
                        ...
                        If idclause.Contains("emptid") = True Then
                    ...
                    Try
                        ...
                        ' salx = fm.getinfo2(sql, Session("con"))
                        ' Response.Write(salx & "====>" & emptid & "<br>")
                        If IsNumeric(taxsum) = False Then
                            taxsum = 0
                        End If
                        sal = IIf(fm.getinfo2(sql, Session("con")) <> "None", fm.getinfo2(sql, Session("con")), fm.getinfo2("select basic_salary from emp_sal_info where emptid=" & emptid & " order by date_start desc", Session("con")))
                        ' Response.Write(sal & "====>" & emptid & "<br>")
                        ...
                        bot = fm.getinfo2("select sum(ot) from payrollx where  " & idclause & " pddate between '" & Request.Form("pd1") & "' and '" & Request.Form("pd2") & "' and tax >0 and  remark='OT-Payment' ", Session("con"))
                        'If emptid = 806 Then
                        'Response.Write(bot & "<br>")
                        'Response.Write("select sum(ot) from payrollx where  " & idclause & " pddate between '" & Request.Form("pd1") & "' and '" & Request.Form("pd2") & "' and tax >0 and  remark='OT-Payment'<br> ")
                        'End If
                        If IsNumeric(bot) = False Then
                            bot = 0
                        End If
                        taxincome = CDbl(taxincome) + CDbl(bot)
                        ...
                        ' oinc = IIf(IsNumeric(fm.getinfo2("select sum(txinco) from payrollx where  " & idclause & " pddate between '" & Request.Form("pd1") & "' and '" & Request.Form("pd2") & "' and remark='Increament'", Session("con"))), fm.getinfo2("select sum(txinco) from payrollx where  " & idclause & " pddate between '" & Request.Form("pd1") & "' and '" & Request.Form("pd2") & "' and remark='Increament'", Session("con")), 0)
                        oinc = fm.getinfo2("select sum(txinco) from payrollx where  " & idclause & " pddate between '" & Request.Form("pd1") & "' and '" & Request.Form("pd2") & "' and remark='Increament' and netp>0", Session("con"))
                        '   Response.Write("select sum(txinco) from payrollx where  " & idclause & " pddate between '" & Request.Form("pd1") & "' and '" & Request.Form("pd2") & "' and remark='Increament' and netp>0<br>")
                        ...
                        If IsNumeric(oinc) = False Then
                            oinc = 0
                        Else
                            ' Response.Write(oinc & "<br>")
                        End If
                        If IsNumeric(netpay) = False Then
                            netpay = 0
                        End If
       ...
       " </tr>" & Chr(13)
        ...
        totall = 0
        ' sumable &="<style>td { left-border:solid gray 1px;}</stype
        For kp As Integer = 0 To UBound(arrproject) - 1
            Try
                Dim ccolor As String = "#3478fa"
                ...
                sumtable &= "<tr style='border:2px solid gray;background-color:#0012fa;'><td colspan=5><b>" & fm.getinfo2("select project_name from tblproject where project_id='" & arrproject(kp) & "'", Session("con")) & "</b></td><td></td></tr>"
                For xx As Integer = 0 To UBound(sxpl)
                    ...
                    '  Response.Write(remarkx)
                    If remarkx = "OT-Payment" Then
                        subrtn = "<td>OT</td><td style='text-align:right'>" & FormatNumber(fm.getinfo2("select sum(ot) from payrollx where ref='" & sxpl(xx) & "'", Session("con")), 2, TriState.True, TriState.True, TriState.True) & "</td>"
                    ElseIf LCase(remarkx) = "increament" Then
                        subrtn = "<td>INC</td><td style='text-align:right'>" & FormatNumber(fm.getinfo2("select sum(txinco) from payrollx where ref='" & sxpl(xx) & "'", Session("con")), 2, TriState.True) & "</td>"
                    ElseIf LCase(remarkx) = "pay_inc_middle" Then
                        subrtn = "<td>Middle</td><td style='text-align:right'>" & FormatNumber(fm.getinfo2("select sum(txinco) from payrollx where ref='" & sxpl(xx) & "'", Session("con")), 2, TriState.True, TriState.True, TriState.True) & "</td>"

                    Else
                        subrtn = "<td>&nbsp;</td><td style='text-align:right'>" & FormatNumber(fm.getinfo2("select sum(txinco) from payrollx where ref='" & sxpl(xx) & "'", Session("con")), 2, TriState.True, TriState.True, TriState.True) & "</td>"
                    End If
                    ...
                    groupt += sumtax
                    sumtable &= "<tr style='border-bottom:1px solid gray;background-color:" & ccolor & ";'><td><a href='rptpayroll.aspx?prid=" & sxpl(xx) & "' target=blank>" & sxpl(xx) & " </a></td>" & subrtn & "<td style='text-align:right;'> " & FormatNumber(sumtax, 2, TriState.True) & " </td><td></td></tr>"
                Next
                If groupt > 0 Then
            ...
            Catch exx As Exception
                Response.Write("errrrr: " & exx.ToString & "<br>" & fm.getinfo2("select sum(ot) from payrollx where ref='" & ref & "'", Session("con")))
            End Try
        Next
