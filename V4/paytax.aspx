<%@ Page Language="VB" AutoEventWireup="false" CodeFile="paytax.aspx.vb" Inherits="paytax" %>
        ...
        If Request.QueryString.HasKeys = True Then
            If Request.QueryString("dox") = "" Then
                ...
                ' Response.Write("<script type='text/javascript'>alert('saving....');</script>")
                                ...
                                'If sysd.isPublic(d1, Session("con")) = False Then
                sql = "insert into tbltax(pdate,path,remark) " & _
                           "values('" & Request.QueryString("pdate") & "','" & Request.QueryString("path") & "','" & _
                           Request.QueryString("remark") & "')" & Chr(13)
                'Response.Write(sql)
                            If sql <> "" Then
...
<link rel="stylesheet" href="jqq/themes/base/jquery.ui.all.css" />
    ...
    <script type="text/javascript"  src="jqq/ui/jquery.ui.tabs.js"></script>
...
'Response.Write("<br />" & Request.Form("do"))
 ...
 <script type="text/javascript">
    ...
    var id;
    var focused = "";
    var requf = ["pdate", "path", "remark", "x"];
    var fieldlist = ["pdate", "path", "remark", "x"];
    function validation1() {
        if ($('#pdate').val() == '') { showMessage('pdate cannot be empty', 'pdate'); $('#pdate').focus(); return false; }
        if ($('#path').val() == '') { showMessage('path cannot be empty', 'path'); $('#path').focus(); return false; }
        if ($('#remark').val() == '') { showMessage('remark cannot be empty', 'remark'); $('#remark').focus(); return false; }
            ...
            if (ans != true) {
                ...
                var str = $("#frmtbltax").formSerialize();
                $("#frmtbltax").attr("action", "?tbl=tbltax&task=<% response.write(keyp) %>&lrd=empcontener&key=id&keyval=<% response.write(idx) %>&rd=empcontener.aspx&" + str);
                $("#frmtbltax").submit();
                return true;
            }
        }
...
<form method='post' id='frmtbltax' name='frmtbltax'> 
<table><tr><td>pdate<sup style='color:red;'>*</sup></td><td>:</td><td><input type='text' id='pdate' name='pdate' value='' /><br /><label class='lblsmall'>
</label></td>
<script language='javascript' type='text/javascript'>    $(function () { $("#pdate").datepicker({ changeMonth: true, changeYear: true }); $("#pdate").datepicker("option", "dateFormat", "mm/dd/yy"); });</script><td>path<sup style='color:red;'>*</sup>
</td><td>:</td><td><input type='file' id='path' name='path' value='' /><br /><label class='lblsmall'></label></td>
</tr><tr><td>remark<sup style='color:red;'>*</sup></td><td>:</td><td><textarea cols='30' rows='10' id='remark' name='remark'>-</textarea><br /><label class='lblsmall'></label></td>
 ...
 <div id="listx" style=" width:100%; padding:0px 7px 0px 0px; font-size:10px;">
    <%  Dim db As New dbclass
        Dim dt As DataTableReader
        If keyp = "update" Then
          
      dt = db.dtmake("new" & Today.ToLocalTime, "select * from emp_att where id=" & Request.QueryString("id"), session("con"))
      If dt.HasRows = True Then
        ...
        Dim sqlx As String = "select id,pdate,path,remark  from tbltax order by pdate desc"
        row = edit_del_list("tbltax", sqlx, "Tax Date,Path,Remark", Session("con"), loc)
        Response.Write(row)
    ...
    <script type="text/javascript">
        function del(val, ans, hd) {
            ...
            if (ans == "yes") {
                ...
                $('#frmx').attr("action", "<% response.write(loc) %>?task=delete&id=" + val + "&tbl=emp_att");
                $('#frmx').submit();
            }
            else {
                ha(hd);
