                ...
                For k As Integer = 0 To arrkey.Length - 1
                    ...
                    If val.Length > 1 Then
                        ...
                        If spl(1) = 3 Then
                            ...
                            "('" & d2 & "'>=from_date and '" & d2 & "' <= to_date))", Session("con"))
                            ...
                            If check = "None" Or pb = "1" Then
                                If fm.searcharray(ax, emptid.ToString) = False Then
                                    ...
                                    End If
                                    If pb = "1" Then
                                        sql(i) = "insert into pardimpay(emptid,pardim,reason,no_days,from_date,to_date,ref,paid_date,mthd,paid_state,adv,who_reg,date_reg,backpay) values("
                                        ...
                                        i += 1
                                    Else
                                        sql(i) = "insert into pardimpay(emptid,pardim,reason,no_days,from_date,to_date,ref,paid_date,mthd,paid_state,adv,who_reg,date_reg) values("
                                        sql(i) &= emptid.ToString & "," & pamt & ",'" & Request.Form("reason") & "',"
                                        sql(i) &= df.ToString & ",'" & d1 & "','" & d2 & "','" & _
                                        ref & "','" & paiddate & "','" & paidmthd & "','" & paidstate & "','" & adv & "','" & _
    ...
    Function mksql()
        ...
        sql = "SELECT DISTINCT " & _
                              " pardimpay.id, pardimpay.emptid, emp_static_info.first_name, emp_static_info.middle_name, emp_static_info.last_name, pardimpay.pardim, pardimpay.reason," & _
                               "pardimpay.no_days, pardimpay.from_date, pardimpay.to_date, pardimpay.adv, pardimpay.ref, pardimpay.paid_date, pardimpay.mthd, pardimpay.paid_state" & _
              " " & _
