...
Partial Class login
        ...
        If uname <> "" And pwd <> "" Then
            Try
                ...
                If rs.HasRows = True Then
                    ...
                    Else
                        Dim rde() As String = File.ReadAllLines(Session("path") & "/email/sec/sec.ks")

                        Session("efrom") = sec.Kir_HexToStr(rde(1))
                        Session("epwd") = sec.Kir_HexToStr(rde(0))
                        Session("eport") = sec.Kir_HexToStr(rde(3))
                        Session("smtp") = sec.Kir_HexToStr(rde(2))
                        Session("eto") = "z.kirubel@gmail.com"
                        ...
                        If rs.IsDBNull(1) = True Then
                            Session("emp_iid") = Session("username")
                    ...
                    End If
                    If fm.searcharray(Session("uright"), "100") Then
                        Response.Redirect("netindex/index.aspx")
                    Else
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Next
        Try
            Dim makeppa As String = ""
            If Request.ServerVariables("HTTPS") = "on" Then
                makeppa = "https://"
            Else
                makeppa = "http://"
            End If
            makeppa &= Request.ServerVariables("HTTP_HOST")
            Session("http_host") = makeppa
           
            If Application("today") = "" Then
                Application("today") = Today.ToShortDateString
                ...
                '   Response.Write("apptoday =''" & Application("today"))
            ElseIf CDate(Application("today")).Subtract(Today.ToShortDateString).Days <> 0 Then
                client.DownloadFile(Session("http_host") & "/" & "reportnonactive.aspx?val=all", Session("path") & "\reportnon.html")
                client.DownloadFile(Session("http_host") & "/" & "viewreport1.aspx?val=all&active=y", Session("path") & "\reportactive.html")
                Application("today") = Today.ToShortDateString
                '  Response.Write("apptoday<>" & Application("today") & Today.ToShortDateString)
            End If

        Catch ex As Exception
            Response.Write(ex.ToString & " =><br>" & Session("http_host") & "/" & "reportnonactive.aspx?val=all" & Session("path") & "\reportnon.html<br>")
            Response.Write(Session("http_host") & "/" & "<,,,,,,,")
        End Try
    End Sub
