<!--@ Page Language="VB" AutoEventWireup="false" CodeFile="payrolltax.aspx.vb" Inherits="payrolltax" %-->
<%@ Page Language="VB" AutoEventWireup="false" CodeFile="taxpension.aspx.vb" Inherits="taxpension" %>
...
Payroll Tax Report
    <form name="frmlistout" action="" method="post" style="font-size:12pt;">
        ...
        <script>            
        $(function() {$( '#pd1').datepicker({changeMonth: true,changeYear: true,maxDate:'+1d'	});
        ...
        <input type="hidden" name="projname" id="projname" value='<%=namelist %>' />
         ...
         <input type="button" value="Go" id="firstform" name="firstform" onclick="javascript:subptax();" />
    </form>
    </div>
    ...
    <div id="pinta4"  style="display:none;width:100px; height:28px;background:url(images/blue_banner-760x147x.jpg)
           #224488;color:White;cursor:pointer;float:left;"
            onclick="javascirpt:printtax('bigprint','Report_print','<% response.write(Session("company_name")) %>','<% response.write(Today.ToLongDateString) %>','printA4');">
