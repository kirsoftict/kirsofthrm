...
Partial Class Default2
    ...
    Public Function makeformunpaid()
        ...
        Dim projid As String = ""
            ...
            Dim pro() As String
            Dim prs As DataTableReader
            prs = dbs.dtmake("vwtblproject", "select * from tblproject", Session("con"))
            If prs.HasRows Then

                While prs.Read
                    ReDim pro(kj + 1)
                pro(kj) = prs.Item("project_name") & "|" & prs.Item("project_id")
                Response.Write("<br>" & pro(kj) & "<br>")
                    kj = kj + 1
        ...
        Dim yyx As Integer = 2019
        Dim mmx As Integer = 1
        For kt As Integer = 0 To 3
            If pro(kt) <> "" Then
                spl = pro(kt).Split("|")
                If kt = 0 Then
                    outp &= "<table style='background:blue;'><tr><td colspan='15'> " & pro(kt) & "</td></tr><tr>"
                Else
                    outp &= "</tr></table><table style='background:blue;'><tr><td colspan='15'>" & pro(kt) & "</td></tr><tr>"
                End If
                If spl.Length > 1 Then
                    projid = spl(1) 'fm.getinfo2("select project_id from tblproject where project_name='" & Request.Form("projname") & "' order by Project_end", session("con"))
            ...
            Else
                If kt = kj Then
                    Exit For
                Else
                    Response.Write("<br>for split error" & kj & "<br>")
                End If
            End If
            For mmxx As Integer = 1 To mmx
                outp &= "<br>.............................month " & mmxx & "<br>"


                nod = Date.DaysInMonth(yyx, mmxx)
                pdate1 = mmxx & "/1/" & yyx
                pdate2 = mmxx & "/" & nod & "/" & yyx
                Dim rs As DataTableReader
                Dim rs2 As DataTableReader
                ...
                'Response.Write("ddd")
                If rs.HasRows Then
                    outp &= "<td colspan='15'><table>" & Chr(13)
                    outp &= "<tr style='text-align:center;font-weight:bold;font-size:1pt' >" & Chr(13)
                    outp &= "<td style='text-align:center;font-weight:bold;' >" & Session("company_name") & _
                "<br> Project Name:"
                    If projid <> "" Then
                        outp &= spl(0).ToString
                    Else
...
"<td style='width:15%;border-style:none none none none;' colspan='2'  align='center'>________________<br>Date</td></tr></table></td>"
    ...
    Function selectout()
        ...
        Dim out As String
        Dim sql As String
        rs = dbs.dtmake("vwempot", "select * from emp_ot where ref is NULL AND paidstatus='y' and ot_date>'7/1/2019' order by ot_date", Session("con"))
        If rs.HasRows Then
            out = "<table border=1>"
            While rs.Read
                out &= "<tr>"
                out &= "<td>" & rs.Item("id") & "</td>"
                out &= "<td>" & fm.getfullname(rs.Item("emp_id"), Session("con")) & "</td>"
                out &= "<td>" & fm.getproj_on_date(rs.Item("emptid"), rs.Item("ot_date"), Session("con"))(1) & "</td>"
                out &= "<td>" & rs.Item("ot_date") & "</td>"
                out &= "<td>" & rs.Item("emp_id") & "</td>"
                out &= "<td>" & rs.Item("who_reg") & "</td></tr>"
    ...
    Function email()
        Try
            ...
            SmtpServer.Credentials = New Net.NetworkCredential("kirsoftet@gmail.com", "493 15kir")

            Dim mail As New MailMessage()

            mail = New MailMessage()
            mail.From = New MailAddress("kirsoftet@gmail.com")
            mail.To.Add("z.kirubel@gmail.com")
