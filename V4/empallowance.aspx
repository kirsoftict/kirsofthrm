<%@ Page Language="VB" AutoEventWireup="false" CodeFile="empallowance.aspx.vb" Inherits="empallowance" %>
...
var id;
var focused="";
var requf=["emp_id","allownace_type","amount","istaxable","from_date","date_reg","active","x"];
var fieldlist=["emp_id","allownace_type","amount","istaxable","from_date","to_date","who_reg","date_reg","active","x"];
function validation1(){if ($('#emp_id').val() == '') {showMessage('emp_id cannot be empty','emp_id');$('#emp_id').focus();return false;}
if ($('#allownace_type').val() == '') {showMessage('allownace_type cannot be empty','allownace_type');$('#allownace_type').focus();return false;}
if ($('#amount').val() == '') {showMessage('amount cannot be empty','amount');$('#amount').focus();return false;}
if ($('#istaxable').val() == '') {showMessage('istaxable cannot be empty','istaxable');$('#istaxable').focus();return false;}
if ($('#from_date').val() == '') {showMessage('from_date cannot be empty','from_date');$('#from_date').focus();return false;}
...
if(ans!=true){ 
 $("#" + ans).focus();
   ...
   var str=$("#frmemp_alloance_rec").formSerialize();
   $("#frmemp_alloance_rec").attr("action","?tbl=emp_alloance_rec&task=<% response.write(keyp) %>&lrd=empcontener&key=id&keyval=<% response.write(idx) %>&rd=empcontener.aspx&" + str);
    $("#frmemp_alloance_rec").submit();
  return true;}
  }
} 
<% if session("emptid").tostring <>"" then %>
listall=[<% response.write(fm.getalllist(session("con"),"emp_alloance_rec","allownace_type")) %>];
<% end if %>
</script>
<form method='post' id='frmemp_alloance_rec' name='frmemp_alloance_rec' action=""> 
...
Allowance Type<sup style='color:red;'>*</sup></td><td>:</td><td colspan="2">
<div style="font-size:12pt;"><input type='text' id='allownace_type' name='allownace_type' value=''/></div><br />
<label class='lblsmall'></label></td><td>&nbsp</td><td></td>
</tr><tr><td>Amount<sup style='color:red;'>*</sup></td><td>:</td><td><input type='text' id='amount' name='amount' value='' /><br /><label class='lblsmall'></label></td>
<td>Taxable<sup style='color:red;'>*</sup></td><td>:</td><td>
<select id="istaxable" name="istaxable">
<option value="n" selected="selected">No</option>
<option value="y">Yes</option></select>
<br /><label class='lblsmall'></label></td>
</tr><tr><td>From Date<sup style='color:red;'>*</sup></td><td>:</td><td>
<input type='text' id='from_date' name='from_date' value='' /><br /><label class='lblsmall'></label></td>
<script language='javascript' type='text/javascript'> $(function() {$( "#from_date").datepicker({changeMonth: true,changeYear: true	}); $( "#from_date" ).datepicker( "option","dateFormat","mm/dd/yy");});</script>
<td>Up to<sup style='color:red;'></sup></td><td>:</td><td>
<input type='text' id='to_date' name='to_date' value='' /><br /><label class='lblsmall'></label></td>
<script language='javascript' type='text/javascript'> $(function() {$( "#to_date").datepicker({changeMonth: true,changeYear: true	}); $( "#to_date" ).datepicker( "option","dateFormat","mm/dd/yy");});</script></tr><tr>
</tr><tr><td>
...
active<sup style='color:red;'>*</sup></td><td>:</td><td><input type='text' id='active' name='active' value='y' />
<br /><label class='lblsmall'></label></td>
...
<input type='reset' onclick="javascript:$('#btnSave').attr('title','Save');$('#btnSave').attr('value','Save');" /></td></tr></table></form>
 ...
 <div id="listx" style=" width:100%; padding:0px 7px 0px 0px;">
    <%  Dim db As New dbclass
      ...
      dt = db.dtmake("new" & Today.ToLocalTime, "select * from emp_alloance_rec where id=" & Request.QueryString("id"), session("con"))
      If dt.HasRows = True Then
        ...
        Dim sqlx As String = "select id,allownace_type,amount,istaxable,from_date,to_date from emp_alloance_rec where emp_id='" & Session("emp_id") & "' order by id desc"
        row = mk.edit_del_list("emp_alloance_rec", sqlx, "Allowance type, Amount, Taxable, From start,End Date", Session("con"), loc)
            Response.Write(row)
      ...
      function del(val,ans,hd)
       {
        ...
        $('#frmx').attr("action","<% response.write(loc) %>?task=delete&id="+val+"&tbl=emp_alloance_rec");
        $('#frmx').submit();
            }
            else
            {
