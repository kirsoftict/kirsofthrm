<%@ Page Language="VB" AutoEventWireup="false" CodeFile="payrollmiddv2.aspx.vb" Inherits="payrollmiddv2" %>
...
%> 
<div id="st"></div>
<div style="width:100%; height:90px; background:#6879aa; text-align:center;color:White; font-size:16pt;">Payroll Maker(Salary Increament in the middle of the month)
    <form name="frmlistout" action="" method="post" style="font-size:12pt;">
        ...
        <select id="res" name="res" style="visibility:hidden;">
         ...
         <input type="button" value="Go" id="firstform" name="firstform" onclick="javascript:submittop2();" />
    </form>
    </div>
    ...
    <input type="button" value='Delete' id='btndel' name='btndel' onclick="javascript: if(confirm('are you sure, Do you want delete all the Payroll data')==true){ deleted();}" style="height:28px; background:url(images/blue_banner-760x147x.jpg) #224488;color:White; font-size:9px;" />
      ...
      <div id="Span1"  style="width:100px; height:28px;background:url(images/blue_banner-760x147x.jpg) #224488;color:White;cursor:pointer;float:left;" onclick="javascirpt:print('bigprint','Report_print','<% response.write(Session("company_name")) %>','<% response.write(Today.ToLongDateString) %>','printA4');">
...
<div id='mkstx' style=" height:28px; width:130px; background:url(images/blue_banner-760x147x.jpg) #224488;color:White; cursor:pointer; display:none; text-align:center; vertical-align:middle; float:left; margin-bottom:auto;" onclick='javascript:findidxv2()'> 
...
Make Statment</span></div> 
       <div style="width: 5px; float: left;">
           ...
           Dim obj As Object
                    ...
                    Dim loc As String = Server.MapPath("download") & "\middlepayroll.txt"
           loc = loc.Replace("\", "/")
           'Response.Write(loc)
    ...
    <input type="hidden" name="fileloc" id="fileloc" value="" />
       <input type="hidden" name="filetype" id="filetype" value="" />
        ...
        <div class="clickexp" style=" float:left; border:none; width:150px;height:28px; background:url(images/blue_banner-760x147x.jpg) #224488;color:White; cursor:pointer;" onclick='javascript:exportx("payrol(<%response.write(now.day.tostring & now.month.tostring & now.year.tostring) %>)","xls","<%response.write(loc) %>","export","2;3");' >
       ...
       ' cod = sec.StrToHex(cod)
        ...
        <%  If Request.QueryString("first") = "" Then
            ...
            If paidflg = True Then
                ...
                xprint &= "$(function() {$( '#paydx').datepicker({changeMonth: true,changeYear: true,maxDate:'+1M'	});" & Chr(13) & " $( '#paydx' ).datepicker( 'option','dateFormat','mm/dd/yy');});" & Chr(13) & "</script>" & Chr(13) & _
                  "Paid Date:<input type='text' name='ppdate' id='ppdate' value='" & _
                  Today.ToShortDateString & "'>" & _
                             "<script language='javascript' type='text/javascript'>$('#bankname').val('CBE');//sumcolx(); " & Chr(13) & _
         "$(function() {$( '#ppdate').datepicker({changeMonth: true,changeYear: true,maxDate:'+1M'	});" & Chr(13) & " $( '#ppdate' ).datepicker( 'option','dateFormat','mm/dd/yy');});" & Chr(13) & "</script>" & Chr(13) & _
                  "<input type='button' id='post' onclick='javascript:findidv2()' name='post' value='Paid/Payroll Date' />"
