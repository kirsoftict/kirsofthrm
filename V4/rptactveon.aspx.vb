...
Partial Class rptactveon
    ...
    Function getout()
        ...
        If Request.QueryString.HasKeys = True Then
            ...
            End If
            valx = Request.QueryString("ppdate")
            Dim pd As Date = "#1/1/1900#"
            If IsDate(valx) Then
                pd = valx
                valx = "allacondet"
            ...
            Select Case valx
                ...
                Case "allacondet"
                    sql = "SELECT emprec.id, emprec.emp_id, emprec.type_recuritment, emprec.hire_date, emprec.end_date, emprec.holiday, emprec.who_reg, " & _
                     " emprec.date_reg, emprec.active FROM emprec INNER JOIN emp_static_info ON emprec.emp_id = emp_static_info.emp_id" & _
                     " where '" & pd & "' between emprec.hire_date and isnull(emprec.end_date,'" & Today.ToShortDateString & "') ORDER BY emp_static_info.first_name, emprec.id DESC"
                    out = headermk("first", "<br>List of Emplyee's on  " & MonthName(Month(pd)) & " " & Day(pd) & ", " & Year(pd))
                    out &= makeformx(sql)
                    out &= "</table>"
                Case "bydep"
                ...
                Case "namex"
                    ...
                    If name.Length > 3 Then
                        If name(0).ToString <> " " Then
                            sql = "SELECT emprec.id, emprec.emp_id, emprec.type_recuritment, emprec.hire_date, emprec.end_date, emprec.holiday, emprec.who_reg, " & _
                    " where emprec.emp_id in(select emp_id from emp_static_info where first_name like '%" & name(0) & "%') and '" & pd & "' between emprec.hire_date and isnull(emprec.end_date,'" & Today.ToShortDateString & "')  order by emprec.id DESC"
            ...
            If dt.HasRows Then
                ...
                While dt.Read
                    ...
                    If fm.searcharray(empid, dt.Item("emp_id")) = False Then
                        ...
                        Dim cnt As String = ""
                        cnt = fm.getinfo2("select count(id) from emprec where emp_id='" & dt.Item("emp_id") & "'", Session("con"))
                        col(0) = (i + 1).ToString
                        ...
                        active = fm.getinfo2("select active from emprec where emp_id='" & dt.Item("emp_id") & "' and end_date is null order by id desc", Session("con"))

                        ' Response.Write(active)
                        If active = "None" Then
                            activedate = fm.getinfo2("select end_date from emprec where emp_id='" & dt.Item("emp_id") & "' order by id desc", Session("con"))
                        ...
                        End If
                        If CInt(cnt) > 1 Then
                            activedate &= " No of recruited>" & cnt

                        End If
                        out &= "<tr style='background-color:" & color & "' title='" & activedate & "'>" & Chr(13)
                        ' Response.Write(active & "<br>")
                        Dim ssstyle As String = ""
                        '  Response.Write(color)
                        For p As Integer = 0 To 15
                            ssstyle = ""
                            If IsNumeric(col(p)) = True Then
                                ssstyle = " style='text-align:right; '"
                            End If
                            If rowspan > 0 And (p >= 8 Or p <= 3) Then


                                If p = 0 Then
                                    out &= "<td rowspan='" & (rowspan).ToString & "' class='dw' " & ssstyle & ">" & col(p) & "</td>" & Chr(13)
                                ElseIf p = 1 Then
                                    out &= "<td rowspan='" & (rowspan).ToString & "' class='fxname'" & ssstyle & ">" & col(p) & "</td>" & Chr(13)
                                ElseIf p = 2 Then
                                    out &= "<td rowspan='" & (rowspan).ToString & "' class='dw'" & ssstyle & ">" & col(p) & "</td>" & Chr(13)
                                ElseIf p = 3 Then
                                    out &= "<td rowspan='" & (rowspan).ToString & "' class='yr'" & ssstyle & ">" & col(p) & "</td>" & Chr(13)
                                ElseIf p = 8 Then
                                    out &= "<td rowspan='" & (rowspan).ToString & "' class='inst'" & ssstyle & ">" & col(p) & "</td>" & Chr(13)
                                ElseIf p > 8 And p < 14 Then
                                    out &= "<td rowspan='" & (rowspan).ToString & "' class='amt'" & ssstyle & ">" & col(p) & "</td>" & Chr(13)
                                    ' Response.Write(col(p) & "<br>")
                                Else

                                    out &= "<td rowspan='" & (rowspan).ToString & "' class='inst'>" & col(p) & "</td>" & Chr(13)

                                End If
                            ElseIf p > 3 And p < 8 Then
                                If rowspan = 0 Then
                                    out &= "<td class='inst'><a href='?val=bydis&discipline=" & fld(4, 0) & "&active=" & Request.QueryString("active") & "' target='frm_tar' style=' color:#0025ff;text-decoration:none'>" & fld(4, 0) & "</a></td>" & Chr(13)
                                    out &= "<td class='inst'><a href='?val=byqual&qualification=" & fld(1, 0) & "&active=" & Request.QueryString("active") & "' target='frm_tar' style=' color:#0025ff;text-decoration:none'>" & fld(1, 0) & "</a></td>" & Chr(13)
                                    out &= "<td class='yr'>" & fld(2, 0) & "</td>" & Chr(13)
                                    out &= "<td class='yr'>" & fld(3, 0) & "</td>" & Chr(13)
                                    p = 8
                                Else
                                    out &= "<td class='inst' ><a href='?val=bydis&discipline=" & fld(4, 0) & "&active=" & Request.QueryString("active") & "' target='frm_tar' style=' color:#0025ff;text-decoration:none'>" & fld(4, 0) & "</a></td>" & Chr(13)
                                    out &= "<td class='inst' ><a href='?val=byqual&qualification=" & fld(1, 0) & "&active=" & Request.QueryString("active") & "' target='frm_tar' style=' color:#0025ff;text-decoration:none'>" & fld(1, 0) & "</a></td>" & Chr(13)
                    ...
                    If rowspan > 1 Then
                        For k As Integer = 1 To CInt(fld(0, 0)) - 1
                                out &= "<tr>" & _
                                "<td class='inst'><a href='?val=bydis&discipline=" & fld(4, k) & "&active=" & Request.QueryString("active") & "' target='frm_tar' style=' color:#0025ff;text-decoration:none'>" & fld(4, k) & "</a></td>" & Chr(13)
                                out &= "<td class='inst'><a href='?val=byqual&qualification=" & fld(1, k) & "&active=" & Request.QueryString("active") & "' target='frm_tar' style=' color:#0025ff;text-decoration:none'>" & fld(1, k) & "</a></td>" & Chr(13)
                                out &= "<td class='yr'>" & fld(2, k) & "</td>" & Chr(13)
                                out &= "<td class='yr'>" & fld(3, k) & "</td></tr>" & Chr(13)
    ...
    Function headermk(ByVal see As String, ByVal title As String)
        ...
        rt = "<table id='tb1' border='0' cellpadding='0' cellspacing='0' class='table table-bordered table-striped table-fixed-header'><thead  class='header'>" & Chr(13) & _
            "" & _
   ...
   " <tr  style='height: 15.75pt; font-size:12pt; color:#020202; font-weight:bold;background-color: #367898'>" & Chr(13) & _
       ...
       " <td class='dw' rowspan='2' style='font-size: 10pt' >" & Chr(13) & _
            "" & Chr(13) & _
           " sex</td>" & Chr(13) & _
        "<td class='yr' rowspan='2' >" & Chr(13) & _
           " Employment date</td>" & Chr(13)
        If see = "second" Then
            rt &= "<td class='yr' rowspan='2'>" & Chr(13) & _
                       " Probation date</td>" & Chr(13)
        End If


        rt &= "<td class='inst' rowspan='2'>" & Chr(13) & _
            "Field of study</td>" & Chr(13) & _
        "<td class='inst' rowspan='2'>" & Chr(13) & _
           " Award</td>" & Chr(13) & _
        "<td class='yr' rowspan='2'>" & Chr(13) & _
       ...
       " <td class='amt' rowspan='2' >" & Chr(13) & _
            "Salary</td>" & Chr(13) & _
        "<td class='inst' colspan='2' >" & Chr(13) & _
        ...
        "<td class='inst' rowspan='2' >" & Chr(13) & _
            "Tel/Mob</td>" & Chr(13) & _
   " </tr>" & Chr(13) & _
    "<tr style='font-weight: bold;color:#020202; font-weight:bold;background-color: #367898'>" & Chr(13) & _
        "<td class='amt'>" & Chr(13) & _
        "Non" & Chr(13) & _
            "<br />" & Chr(13) & _
            "Taxable</td>" & Chr(13) & _
        "<td class='amt'>" & Chr(13) & _
