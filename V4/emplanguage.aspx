<%@ Page Language="VB" AutoEventWireup="false" CodeFile="emplanguage.aspx.vb" Inherits="emplanguage" %>
...
var id;
var focused="";
var requf=["emp_id","language","level_speak","level_read","level_write","x"];
var fieldlist=["emp_id","language","level_speak","level_read","level_write","who_reg","date_reg","publish","x"];
function validation1(){if ($('#emp_id').val() == '') {showMessage('emp_id cannot be empty','emp_id');$('#emp_id').focus();return false;}
if ($('#language').val() == '') {showMessage('language cannot be empty','language');$('#language').focus();return false;}
if ($('#level_speak').val() == '') {showMessage('level_speak cannot be empty','level_speak');$('#level_speak').focus();return false;}
if ($('#level_read').val() == '') {showMessage('level_read cannot be empty','level_read');$('#level_read').focus();return false;}
if ($('#level_write').val() == '') {showMessage('level_write cannot be empty','level_write');$('#level_write').focus();return false;}
...
if(ans!=true){ 
 $("#" + ans).focus();
   ...
   var str=$("#frmlanguage").formSerialize();
   $("#frmlanguage").attr("action","?tbl=language&task=<% response.write(keyp) %>&lrd=empcontener&key=id&keyval=<% response.write(idx) %>&rd=empcontener.aspx&" + str);
    $("#frmlanguage").submit();
  return true;}
  }
} </script>
...
<span class="close" id="clickclose_s" style="cursor:pointer;"></span>
    ...
    <form method='post' id='frmlanguage' name='frmlanguage' action=""> 
<table><tr><td><input type='hidden' id='emp_id' name='emp_id' value="<%response.write(session("emp_id")) %>" />
Language<sup style='color:red;'>*</sup></td>
<td>Level of Speak<sup style='color:red;'>*</sup></td>
<td>Level of Read<sup style='color:red;'>*</sup></td>
<td>Level of Write<sup style='color:red;'>*</sup></td></tr>
<tr>
<td><input type='text' id='language' name='language' /></td>
<td><select id='level_speak' name='level_speak' ><option value="None">None</option>
...
<td><select id='level_read' name='level_read'>
...
<td><select id='level_write' name='level_write' >
<option value="None">None</option>
<option value="Fair">Fair</option>
<option value="Good">Good</option>
<option value="Very Good">Very Good</option>
...
publish<sup style='color:red;'>*</sup></td><td>:</td><td>
<input type='text' id='publish' name='publish' value='y' /></td>
</tr><tr>
<td colspan='4'><input type='button' name='btnSave' id='btnSave' value='Save' /><input type='reset' onclick="javascript:$('#btnSave').attr('title','Save');$('#btnSave').attr('value','Save');" /></td></tr></table></form>
    </div>
 ...
 <div id="listx" style=" width:100%; padding:0px 7px 0px 0px;">
    <%  Dim db As New dbclass
        ...
        If keyp = "update" Then
            dt = db.dtmake("new" & Today.ToLocalTime, "select * from language where id=" & Request.QueryString("id"), session("con"))
        ...
        Dim sqlx As String = "select id,Language,level_speak,level_read,level_write from language where emp_id='" & Session("emp_id") & "' order by id desc"
        row = mk.edit_del_list("language", sqlx, "Language,Speak,Read,Write", session("con"), loc)
            Response.Write(row)
...
hform=findh(document.getElementById("middle_bar"));
    $('.titlet').text("Language abilities Detail");
      ...
      function del(val,ans,hd)
       {
        ...
        $('#frmx').attr("action","<% response.write(loc) %>?task=delete&id="+val+"&tbl=language");
        $('#frmx').submit();
            }
            else
            {
