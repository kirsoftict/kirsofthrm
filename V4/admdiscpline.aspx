<%@ Page Language="VB" AutoEventWireup="false" CodeFile="addmartialstatus.aspx.vb" Inherits="addmartialstatus" %>
...
<script type="text/javascript">
var requf=["stat"];
var fieldlist=["stat","description","x"];
var prv;
  prv="";
      var id;
  var focused="";
...
if ($('#stat').val() == '') {showMessage('stat cannot be empty','stat');$('#stat').focus();return false;}
  ...
  { 
   ...
   // alert(ans);
    ...
    str=$("#frmmartial_status").formSerialize();
  //alert(str);
  //$("#messagebox").load("open.aspx?tbl=martial_status&task=save&key=stat&rd=&" + str);
     $("#frmmartial_status").attr("action","?tbl=martial_status&task=<% response.write(keyp) %>&lrd=empcontener&key=id&keyval=<% response.write(idx) %>&rd=admincontener.aspx&" + str);
    
   // $("#messagebox").load("open.aspx?tbl=emp_address&rd=&" + str);
    $('#frmmartial_status').submit();
...
<form method='post' id='frmmartial_status' name='frmmartial_status' action=""> 
<table width="400"><tr><td>Martial status<sup style='color:red;'>*</sup></td><td>:</td><td>
<input type='text' id='stat' name='stat' /></td></tr>
<tr><td>Description<sup style='color:red;'>*</sup></td><td>:</td><td><textarea rows="4" cols="50" id='description' name='description' /
></textarea></td>
</tr>
<tr><td colspan="4">
<input type='text' id='date_of_reg' name='date_of_reg' value="<% dim lucur(3) as string
...
<input type='text' id='who_reg' name='who_reg' value="<%response.write(session("username")) %>"/>
<input type="button" name="btnSave" id="btnSave" value="Save" />
 ...
 <div id="listx" style=" width:100%; padding:0px 7px 0px 0px;">
    <%  Dim db As New dbclass
        ...
        If keyp = "update" Then
            dt = db.dtmake("new" & Today.ToLocalTime, "select * from martial_status where id=" & Request.QueryString("id"), session("con"))
            ...
            Dim sqlx As String = "select id,stat,description from martial_status order by id desc"
            row = mk.edit_del_list("martial_status", sqlx, "State,Description", session("con"), loc)
            Response.Write(row)
            ' Response.Write(keyp)
       If keyp = "delete" Then
...
hform=findh(document.getElementById("middle_bar"));
    $('.titlet').text("Add Marital status");
      ...
      function del(val,ans,hd)
       {
        ...
        $('#frmx').attr("action","<% response.write(loc) %>?task=delete&id="+val+"&tbl=martial_status");
        $('#frmx').submit();
            }
            else
            {
