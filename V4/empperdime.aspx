<%@ Page Language="VB" AutoEventWireup="false" CodeFile="empperdime.aspx.vb" Inherits="empperdime" %>
...
var id;
var focused="";
var requf=["emp_id","from_date","date_reg","active","x"];
...
if(ans!=true){ 
 $("#" + ans).focus();
   ...
   var str=$("#frmemp_pardime").formSerialize();
   $("#frmemp_pardime").attr("action","?tbl=emp_pardime&task=<% response.write(keyp) %>&lrd=empcontener&key=id&keyval=<% response.write(idx) %>&rd=empcontener.aspx&" + str);
    $("#frmemp_pardime").submit();
  return true;}
  }
} 
</script>
<form method='post' id='frmemp_pardime' name='frmemp_pardime' action=""> 
...
Per-dime<sup style='color:red;'>*</sup></td><td>:</td><td colspan="2">
<div style="font-size:12pt;"><input type='text' id='pardime' name='pardime' value='' style="text-align:right;"/></div><br />
...
<input type='reset' onclick="javascript:$('#btnSave').attr('title','Save');$('#btnSave').attr('value','Save');" /></td></tr></table></form>
 ...
 <div id="listx" style=" width:100%; padding:0px 7px 0px 0px;">
    <%  Dim db As New dbclass
      ...
      dt = db.dtmake("new" & Today.ToLocalTime, "select * from emp_pardime where id=" & Request.QueryString("id"), session("con"))
      If dt.HasRows = True Then
        ...
        Dim sqlx As String = "select id,pardime,from_date,to_date from emp_pardime where emp_id='" & Session("emp_id") & "' order by id desc"
        row = mk.edit_del_list("emp_pardime", sqlx, "Per-dime Amount, From start,Upto", session("con"), loc)
            Response.Write(row)
      ...
      function del(val,ans,hd)
       {
        ...
        $('#frmx').attr("action","<% response.write(loc) %>?task=delete&id="+val+"&tbl=emp_pardime");
        $('#frmx').submit();
            }
            else
            {
