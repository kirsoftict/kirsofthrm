<%@ Page Language="VB" AutoEventWireup="false" CodeFile="national.aspx.vb" Inherits="national" %>
...
if ($('#country').val() == '') {showMessage('country cannot be empty','country');$('#country').focus();return false;}if ($('#relationship').val() == '') {showMessage('relationship cannot be empty','relationship');$('#relationship').focus();return false;}
else if(focused=="")
  { 
    var ans
    ans=checkblur();
   // alert(ans);
    ...
    str=$("#frmnationality").formSerialize();
  //alert(str);
  //$("#messagebox").load("open.aspx?tbl=nationality&task=save&key=stat&rd=&" + str);
     $("#frmnationality").attr("action","?tbl=nationality&task=<% response.write(keyp) %>&lrd=empcontener&key=id&keyval=<% response.write(idx) %>&rd=admincontener.aspx&" + str);
...
<span class="close" title="Close" id="clickclose_s" style="cursor:pointer;"></span>
 ...
 <form method='post' id='frmnationality' name='frmnationality' action=""> 
<table><tr><td>country<sup style='color:red;'>*</sup></td><td>:</td><td><input type='text' id='country' name='country' /></td>
</tr><tr><td colspan='4'><input type='hidden' id='date_of_reg' name='date_of_reg' value="<% dim lucur(3) as string
...
<input type='button' name='btnSave' id='btnSave' value='Save' /><input type='reset' onclick="javascript:$('#btnSave').attr('title','Save');$('#btnSave').attr('value','Save');" /></td></tr></table></form>
 ...
 <div id="listx" style=" width:100%; padding:0px 7px 0px 0px;">
    <%  Dim db As New dbclass
        ...
        If keyp = "update" Then
            dt = db.dtmake("new" & Today.ToLocalTime, "select * from nationality where id=" & Request.QueryString("id"), session("con"))
            ...
            Dim sqlx As String = "select id,country from nationality order by id desc"
            row = mk.edit_del_list("nationality", sqlx, "Nationalities", session("con"), loc)
            Response.Write(row)
            ' Response.Write(keyp)
       If keyp = "delete" Then
