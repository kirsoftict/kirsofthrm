<%@ Page Language="VB" AutoEventWireup="false" CodeFile="gtaxbook.aspx.vb" Inherits="gtaxbook" %>
...
<link rel="stylesheet" href="jq/themes/ui-lightness/jquery.ui.all.css" />
    ...
    <script type="text/javascript" src="jqq/ui/jquery.number.js"></script>
...
<script src="scripts/script.js" type="text/javascript"></script>
    ...
    <link href="css/payrol.css" media="screen" rel="stylesheet" type="text/css" />
            ...
            <link href="css/taxold.css" media="screen" rel="Stylesheet" type="text/css" />
             <link rel="stylesheet" href="css/Employment Tax_files/fonts.css" type="text/css" media="all">
     <!--link rel="stylesheet" href="css/bootstrap.css" type="text/css" media="all"-->
<link rel="stylesheet" href="css/Employment Tax_files/style.css" type="text/css" media="all">
<link rel="stylesheet" id="screen_css-css" href="css/Employment Tax_files/employment_tax_screen.css" type="text/css" media="screen">
...
if(lac=="signc"){
         ...
         function exportxx2(div, loc, ftype, rigt) {
            //alert(floc);
           /// 
           // alert($("#" + div).html());
           
          // var txtpass=$("#" + div).html();
             $("#tim").load("exporttoexcel.aspx",{'loc':loc,
             'datatra':$('#' + div).html()
             });
             $("#export").attr("action", "print.aspx?exporttax=on");
            $("#filetype").val(ftype);
            $("#fileloc").val(loc);
             $("#rigt").val(rigt);
             $("#filename").val("govtax<%=now.ticks %>.xls");
            $("#export").submit();
...
Payroll Tax Government Report Form
    <form name="frmlistout" action="" method="post" style="font-size:12pt;">
        ...
        <input type="hidden" name="projname" id="projname" value='<%=namelist %>' />
          <select id="paidst" name="paidst" style="display:none";>
...
<div style=" clear:left"></div>
   ...
   <% 
       ...
       ElseIf Request.Form("paidst") = "tax" Then
           ...
           Try
               'Response.Write("WOOOOOOOOOOOOOOOOO")
               ' Dim ref As String
               'ref = fm.getjavalist("payrollx where pddate between '" & Request.Form("pd1") & "' and '" & Request.Form("pd2") & "'", "ref", Session("con"))
               '  ref = fm.getjavalist("payrollx where pddate between '" & Request.Form("pd1") & "' and '" & Request.Form("pd2") & "'", " distinct ref", Session("con"))
               'Response.Write(ref)
               '  ref = ref.Replace(Chr(34), "'")
               '  ref=ref.Replace(" ","")
               '  makepage_all_tax(ref, Request.Form("pd1"), Request.Form("pd2"))
               '  makesummery(CDate(Request.Form("pd1")), CDate(Request.Form("pd2")))
               collectiononly()
           Catch ex As Exception
               '    Response.Write(ex.ToString & Request.Form("pd1") & "==============" & Request.Form("pd2"))
