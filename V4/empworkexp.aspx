<%@ Page Language="VB" AutoEventWireup="false" CodeFile="empworkexp.aspx.vb" Inherits="empworkexp" %>
	...
	$(function() {
		$( "#from_d" ).datepicker({
			...
			onSelect: function( selectedDate ) {
				$( "#to_d" ).datepicker( "option", "minDate", selectedDate );
				
			}
		});
		$( "#to_d" ).datepicker( "option","dateFormat","mm/dd/yy");
		$( "#from_d" ).datepicker( "option","dateFormat","mm/dd/yy");
		$( "#to_d" ).datepicker({
			defaultDate: "+1w",
			changeMonth: true,
			changeYear:true,
			...
			maxDate: 0,
			onSelect: function( selectedDate ) {
				$( "#from_d" ).datepicker( "option", "MaxDate", selectedDate );
			}
		});
	});
	</script>
...
var id;
var focused="";
var requf=["emp_id","organization","from_d","position","job_description","x"];
var fieldlist=["emp_id","organization","from_d","to_d","position","job_description","last_salary","cur","who_reg","date_reg","stat","x"];
function validation1(){if ($('#emp_id').val() == '') {showMessage('emp_id cannot be empty','emp_id');$('#emp_id').focus();return false;}
if ($('#organization').val() == '') {showMessage('organization cannot be empty','organization');$('#organization').focus();return false;}
if ($('#from_d').val() == '') {showMessage('from_d cannot be empty','from_d');$('#from_d').focus();return false;}
if ($('#position').val() == '') {showMessage('position cannot be empty','position');$('#position').focus();return false;}
if ($('#job_description').val() == '') {showMessage('job_description cannot be empty','job_description');$('#job_description').focus();return false;}
...
if(ans!=true){ 
 $("#" + ans).focus();
   ...
   var str=$("#frmtblworkexp").formSerialize();
   $("#frmtblworkexp").attr("action","?tbl=tblworkexp&task=<% response.write(keyp) %>&lrd=empcontener&key=id&keyval=<% response.write(idx) %>&rd=empcontener.aspx&" + str);
    $("#frmtblworkexp").submit();
  return true;}
  }
} </script>
...
<span class="close" id="clickclose_s" style="cursor:pointer;"></span>
    ...
    <form method='post' id='frmtblworkexp' name='frmtblworkexp' action=''> 
<table><tr><td><input type='hidden' id='emp_id' name='emp_id' value="<% response.write(session("emp_id")) %>" />
Organization Name<sup style='color:red;'>*</sup></td><td>:</td>
<td colspan="5"><input type='text' id='organization' name='organization' style="width: 357px" /></td>
</tr><tr>
<td>From<sup style='color:red;'>*</sup></td><td>:</td><td><input type='text' id='from_d' name='from_d' /></td>
<td>To<sup style='color:red;'>*</sup></td><td>:</td><td><input type='text' id='to_d' name='to_d' /></td>
</tr><tr><td >Position<sup style='color:red;'>*</sup>
...
</select></td></tr><tr><td>Employeement Type</td><td>:</td><td>
    <select id='rec_cond' name='rec_cond'>
...
<td>Job Description<sup style='color:red;'>*</sup></td><td>:</td><td colspan="4">
<textarea id='job_description' name='job_description' cols="30" rows="7" ></textarea></td>
</tr><tr><td>Last salary<sup style='color:gray;'>optional</sup></td><td>:</td>
<td><input type='text' id='last_salary' name='last_salary' /></td>
<td>cur<sup style='color:Gray;'>optoinal</sup></td><td>:</td><td>
...
</tr><tr><td>stat<sup style='color:red;'>*</sup></td><td>:</td><td><input type='text' id='stat' name='stat' value='y' /></td>
</tr></tr><tr><td colspan='4'><input type='button' name='btnSave' id='btnSave' value='Save' /><input type='reset' onclick="javascript:$('#btnSave').attr('title','Save');$('#btnSave').attr('value','Save');" />
</td></tr></table></form></div></div>
    <div><sup style="color:Red;">*</sup>Required Fields
 </div>
 <div id="listx" style=" width:100%; padding:0px 7px 0px 0px;">
    <%  Dim db As New dbclass
        Dim dt As DataTableReader
        If keyp = "update" Then
            dt = db.dtmake("new" & Today.ToLocalTime, "select * from tblworkexp where id=" & Request.QueryString("id"), session("con"))
            If dt.HasRows = True Then
                dt.Read()
                Response.Write("<script type='text/javascript'>")
                For k As Integer = 0 To dt.FieldCount - 4
                    If dt.IsDBNull(k) = False Then
                        If dt.GetDataTypeName(k).ToLower = "string" Then
                        %>
        ...
        Dim sqlx As String = "select id,organization,position,from_d,to_d from tblworkexp where emp_id='" & Session("emp_id") & "' order by id desc"
        row = mk.edit_del_list("tblworkexp", sqlx, "Organization Name,Position,From,to", session("con"), loc)
            Response.Write(row)
...
hform=findh(document.getElementById("middle_bar"));
    $('.titlet').text("Work Experience Details");
      ...
      function del(val,ans,hd)
       {
        ...
        $('#frmx').attr("action","<% response.write(loc) %>?task=delete&id="+val+"&tbl=tblworkexp");
        $('#frmx').submit();
            }
            else
            {
