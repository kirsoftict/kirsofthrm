<%@ Page Language="VB" AutoEventWireup="false" CodeFile="qualification.aspx.vb" Inherits="qualification" %>
...
<%  Dim namelist As String = ""
    ...
    namelist = fm.getjavalist2("tblqualification", "qualification", Session("con"), "")
 %>
<script type="text/javascript">
...
var id;
var focused="";
var requf=["qualification","description","x"];
var fieldlist=["qualification","description","who_reg","date_reg","stat","x"];
...
if(ans!=true){ 
 $("#" + ans).focus();
   ...
   var str=$("#frmtblqualification").formSerialize();
   $("#frmtblqualification").attr("action","?tbl=tblqualification&task=<% response.write(keyp) %>&lrd=empcontener&key=id&keyval=<% response.write(idx) %>&rd=empcontener.aspx&" + str);
    $('#frmtblqualification').submit();
  return true;}
  }
} </script>
</head>
...
<div style="width:200px;">
   ...
   Search:</td><td>  <input type='text' name='vname' id='vname' style='font-size:9pt;'  onkeyup="javascript:startwith('vname',namelist);" /><br/>
    <label class='lblsmall'>Write Qualification</label></td>
    <td valign='top'>
    </td></tr></table></form>
    </div>
 <div id="formouterbox">
...
<form method='post' id='frmtblqualification' name='frmtblqualification' action=""> 
<table><tr><td>Qualification<sup style='color:red;'>*</sup></td><td>:</td><td><input type='text' id='qualification' name='qualification' /></td></tr><tr>
<td>Description<sup style='color:red;'>*</sup></td><td>:</td><td><textarea id='description' name='description' cols="30" rows="10" ></textarea></td>
</tr><tr><td>Floating Level<sup style='color:red;'>*</sup></td><td>:</td><td><input type="text" id='hr' name='hr' size:'2' /></td>
</tr><tr><td colspan="4"><input type='hidden' id='who_reg' name='who_reg' value="<%response.write(session("username")) %>" />
...
<input type='hidden' id='stat' name='stat' value="y" />
<input type='button' name='btnSave' id='btnSave' value='Save' /><input type='reset' onclick="javascript:$('#btnSave').attr('title','Save');$('#btnSave').attr('value','Save');" /></td></tr></table></form>
    ...
    <%  Dim db As New dbclass
        ...
        If keyp = "update" Then
            dt = db.dtmake("new" & Today.ToLocalTime, "select * from tblqualification where id=" & Request.QueryString("id"), session("con"))
            ...
            Dim sqlx As String = "select id,qualification,description,hr from tblqualification order by hr"
            row = mk.edit_del_list("tblqualification", sqlx, "Qualification,Description,Floating Level", Session("con"), loc)
            Response.Write(row)
            ' Response.Write(keyp)
       If keyp = "delete" Then
      ...
      function del(val,ans,hd)
       {
        ...
        $('#frmx').attr("action","<% response.write(loc) %>?task=delete&id="+val+"&tbl=tblqualification");
        $('#frmx').submit();
            }
            else
            {
                 $('#frmx').attr("target","_self");
        $('#frmx').attr("action","qualification.aspx?task=&id="+val+"&tbl=tbldiscipline");
        $('#frmx').submit();
