<%@ Page Language="VB" AutoEventWireup="false" CodeFile="emppens.aspx.vb" Inherits="emppens" %>
...
<%  Dim dbc As New dbclass
    ...
    ' listids = dbc.idlist_jqry("emp_pen_rate", "leave_type", session("con"))
...
var id;
var focused="";
var requf=["start_date","p_rate_empr","p_rate_empee","x"];
var fieldlist=["start_date","p_rate_empr","p_rate_empee","who_reg","date_reg","x"];
function validation1(){
if ($('#start_date').val() == '') {showMessage('start_date cannot be empty','start_date');$('#start_date').focus();return false;}
if ($('#p_rate_empr').val() == '') {showMessage('p_rate_empr cannot be empty','p_rate_empr');$('#p_rate_empr').focus();return false;}
if ($('#p_rate_empee').val() == '') {showMessage('p_rate_empee cannot be empty','p_rate_empee');$('#p_rate_empee').focus();return false;}
else if(focused=="") { var ans;
ans=checkblur();
if(ans!=true){ 
 $("#" + ans).focus();
}else{
   var str=$("#frmemp_pen_rate").formSerialize();
   $("#frmemp_pen_rate").attr("action","?tbl=emp_pen_rate&task=<% response.write(keyp) %>&lrd=empcontener&key=id&keyval=<% response.write(idx) %>&rd=empcontener.aspx&" + str);
    $("#frmemp_pen_rate").submit();
  return true;}
  }
} </script>
...
<span class="close" title="Close" id="clikclose" style="cursor:pointer;"></span>
 ...
 <form method='post' id='frmemp_pen_rate' name='frmemp_pen_rate' action=""> 
<table><tr><td>Action Date<sup style='color:red;'>*</sup></td><td>:</td>
<td><input type='text' id='start_date' name='start_date' value='' /><br />
...
<td>Employer Contribution<sup style='color:red;'>*</sup></td><td>:</td><td>
<input type='text' id='p_rate_empr' name='p_rate_empr' value='' size="2" style="text-align:right;" />%<br /><label class='lblsmall'></label></td>
</tr><tr>
<td>Employee Contribution<sup style='color:red;'>*</sup></td><td>:</td><td>
<input type='text' id='p_rate_empee' name='p_rate_empee' value='' size="2" style="text-align:right;" />%<br /><label class='lblsmall'></label>
...
<input type='button' name='btnSave' id='btnSave' value='Save' /><input type='reset' onclick="javascript:$('#btnSave').attr('title','Save');$('#btnSave').attr('value','Save');" /></td></tr></table></form>
 ...
 <div id="listx" style=" width:100%; padding:0px 7px 0px 0px;">
    <%  Dim db As New dbclass
        ...
        If keyp = "update" Then
            dt = db.dtmake("new" & Today.ToLocalTime, "select * from emp_pen_rate where id=" & Request.QueryString("id"), session("con"))
            ...
            Dim sqlx As String = "select id,start_date,p_rate_empr,p_rate_empee from emp_pen_rate order by id desc"
            row = mk.edit_del_list("emp_pen_rate", sqlx, "Action Date,Emplyer <br>Contribution,Empbolyee<br> Contribution", session("con"), loc)
            Response.Write(row)
            ' Response.Write(keyp)
       If keyp = "delete" Then
...
//hform=findh(document.getElementById("middle_bar"));
    $('.titlet').text("Pension Yearly Setup");
      ...
      function del(val,ans,hd)
       {
        ...
        $('#frmx').attr("action","<% response.write(loc) %>?task=delete&id="+val+"&tbl=emp_pen_rate");
        $('#frmx').submit();
            }
            else
            {
                 $('#frmx').attr("target","_self");
        $('#frmx').attr("action","?task=&id="+val+"&tbl=emp_pen_rate");
        $('#frmx').submit();
