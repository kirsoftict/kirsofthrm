...
Partial Class pardimpayformn
    ...
    Function outpx()
        ...
        If Request.Form("projname") <> "" Then
            ...
            ElseIf Request.Form("paidst2") = "no" Then
                mkform(projid, "no2", fdate, edate)
            Else
    ...
    Function mkformview(ByVal projid As String, ByVal fdate As Date, ByVal edate As Date)
        ...
        If fdate <> "1/1/1900" Then
            ...
            rs = ds.dtmake("selectpar", "select * from pardimpay inner join emprec on emprec.id=pardimpay.emptid INNER JOIN emp_static_info AS empsf ON emprec.emp_id = empsf.emp_id " & _
                          "where paid_state='y' and paid_date between '" & fdate & "' and '" & edate & "' order by paid_date,empsf.first_name,pardimpay.ref,pardimpay.id", Session("con"))
            ' Response.Write("select * from pardimpay inner join emprec on emprec.id=pardimpay.emptid INNER JOIN emp_static_info AS empsf ON emprec.emp_id = empsf.emp_id " & _
            '               "where paid_state='y' and paid_date between '" & fdate & "' and '" & edate & "' order by paid_date,empsf.first_name,pardimpay.ref,pardimpay.id")
            '   Response.Write("select * from pardimpay inner join emprec on emprec.id=pardimpay.emptid INNER JOIN emp_static_info AS empsf ON emprec.emp_id = empsf.emp_id " & _
            '                "where paid_state='y' and paid_date between '" & fdate & "' and '" & edate & "' order by paid_date,empsf.first_name,pardimpay.ref,pardimpay.id")

            '  Response.Write("select * from pardimpay inner join emprec on emprec.id=pardimpay.emptid INNER JOIN emp_static_info AS empsf ON emprec.emp_id = empsf.emp_id " & _
            '         "where paid_state='y' and paid_date between '" & fdate & "' and '" & edate & "' order by paid_date,id,empsf.first_name")
        ...
        Else
            ...
            ' rs = ds.dtmake("mmkj", "SELECT pardimpay.* FROM pardimpay INNER JOIN emprec ON pardimpay.emptid = emprec.id INNER JOIN emp_static_info ON emprec.emp_id = emp_static_info.emp_id ORDER BY pardimpay.paid_date,pardimpay.ref, emp_static_info.first_name", Session("con"))
        ...
        If rs.HasRows = True Then
            ...
            While rs.Read
                ...
                paytype = rs.Item("payreason")
               
                If IsError(paytype) Then
                    Response.Write("errrrr" & emptid & "<br>")
                End If

                rt = fm.getproj(emptid, fdate, rs.Item("paid_date"), Session("con"))
                rt = fm.getproj_on_date(emptid, edate, Session("con"))
                    ...
                    If projid = rt(0) Then
                        ...
                        If copy <> refc Then
                            ...
                            If tblmakeflg = True Then
                                header &= "<tr><td colspan='6' style='text-align:center;cursor:pointer; color:red;' onclick=" & Chr(34) & _
                                    "javascript:view('view-" & copy & "');" & Chr(34) & " >View</td>" & Chr(13) & _
                            ...
                            If ppd <> rs.Item("paid_date") Then
                                ...
                                MonthName(ppd2.Month) & " " & ppd2.Day.ToString & ", " & ppd2.Year.ToString & "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a id='flton" & ppd2.Ticks & "' href=" & Chr(34) & "javascript:printp('txt" & ppd2.Ticks & "'); " & Chr(34) & _
                                "> Print all</a><img id='ic-" & rs.Item("paid_date") & "'  src='images/gif/collapsed_.gif' alt='cc' style='float:right;margin:0px 0px 0px 100px;width:17px; ' /></div>" & Chr(13) & _
        ...
        emptid, Session("con")), Session("con")).ToString & _
                            ...
                            "<td>No. Days</td><td>" & paytype & "</td><td>Amount</td></tr>"
                            copy = refc
                            tblmakeflg = True
                        Else
                            ' Response.Write("<tr><td>" & rs.Item("payreason") & "...,</td></tr>")
                        End If
                        ...
                        ' End If
                        refc = ""
                        Dim arx(), arx2() As String
                        arx = isinproject2(emptid, fdate, edate, Session("con")).split(",")
                        If arx.Length >= 1 Then
                            '  Response.Write(emptid & "===" & arx.Length.ToString & "<br>")
                        End If
    ...
    Function mkformps2(ByVal projid As String, ByVal fdate As Date, ByVal edate As Date)
        Dim copy As String = ""
        Dim header As String = ""
        Dim color As String = ""
        Dim amt As Double = 0
        Dim rs As DataTableReader
        Dim sql As String
        Dim eidcol As String = ""
        Dim idcopy As String = ""
        Dim tblmakeflg As Boolean = False
        Try
            ...
            If rs.HasRows = True Then
                ...
                While rs.Read
                    ...
                    If copy <> emptid Then
                        If idcopy <> "" Then
                            idcopy = idcopy.Substring(0, idcopy.Length - 1)

                            idcopy &= ""
                        End If
                        eidcol &= idcopy & "|" & emptid
                        idcopy = "="
                        If color = "#aabbbf" Then
                            color = "#C8DBF0"
                            ' header &= "<tr><td colspan='4' style='text-align:center;'>Paid</td></tr></table>"
                ...
                If eidcol.Length > 2 Then
                    '  Response.Write(eidcol.Length & "<br>" & eidcol & "<br>")
                    eidcol = eidcol.Substring(1, eidcol.Length - 2)
                End If
                Dim newentry As String
                newentry = "<form id='form-all' name='form-all' action='' method='post'><table> <tr> " & _
               "<td colspan='5'><select name='mthd' id='mthd'>" & _
               "<option value='Cheque'>Cheque</option>" & _
               "<option value='Bank'>Bank</option>" & _
                "<option value='Cash'>Cash</option></select>" & _
                "<input type='text' id='paiddate2' name='paiddate2' value='" & Today.Month & _
                "/" & Today.Day & "/" & Today.Year & "'><input type='hidden' name='alldata' id='alldata' value='" & eidcol & "'></td>" & _
                "<script language='javascript' type='text/javascript'> $(function() {$( '#paiddate2').datepicker({changeMonth: true,changeYear: true	}); $( '#paiddate' ).datepicker( 'option','dateFormat','mm/dd/yy');});</script>" & _
                "<td colspan='' style='text-align:center;cursor:pointer; color:red;' onclick=" & Chr(34) & _
                "javascript:findidall();" & Chr(34) & _
                " ><span class='btn'>Paid</span></td></tr></table></form>"
                Response.Write(newentry & "<br>" & header)
            Else
                ' Response.Write("Data is not found")
            End If
        Catch ex As Exception
